...
def is_valid_mobile_number(mobile_number):
    valid_prefixes = {'07', '+447', '00447'}
    return True if mobile_number[1:].isdigit() and mobile_number[:-9] in valid_prefixes else False
...
def get_williams(names):
    return [name for name in names if name[-8:] == 'Williams']
...
def get_factorials(numbers):
    ...
    return [get_factorial(num) for num in numbers]
...
def generate_matrix(number):
    return [[None for col in range(number)] for row in range(number)]
...
def find_wrong_way_fruit(orchard):
    ...
    correct_spelling = max(orchard, key=orchard.count)

    for index, fruit in enumerate(orchard):
        if fruit != correct_spelling:
            return index
...
def dna_pairs(dna_string):
    ...
    return [char + valid_pairs.get(char) 
            for char in dna_string.upper() 
