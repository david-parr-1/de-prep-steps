...
def is_absolute_path(file_path):
    return True if file_path[0] == "/" else False
...
def get_char_code(char):
    return f"The ASCII code for {char} is {ord(char)}"
...
def create_list(length, char):
    return [char for num in range(length)]
